...
public class TaskOperations {
    ...
    {
        Cursor c = database.rawQuery("SELECT * FROM tasks WHERE isActive=1", null);

        if (c != null) {
            c.moveToFirst();
        }

        int tasks = c.getCount();
        return tasks;
    }

    public int getInactiveTasks()
    {
        Cursor c = database.rawQuery("SELECT * FROM tasks WHERE isActive=0", null);
